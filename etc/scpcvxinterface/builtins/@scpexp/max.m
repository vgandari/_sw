% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% FUNCTION: max: Expression of the "max" function
% Date: 09-06-2009
% Create by: Quoc Tran Dinh, ESAT/SCD and OPTEC, KULeuven, Belgium.
% Supervisor: Prof. Moritz Diehl.
% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
function x = max( varargin )

if nargin == 1
    if isa( varargin{1}, 'scpexp' )
        x = varargin{1};
        x.expr  = strcat( 'max(', x.expr,  ')' );
        x.vexpr = strcat( 'max(', x.vexpr, ')' );
    end
elseif nargin == 2
    ...
    if isa( tmp1, 'scpexp' ) || isa( tmp2, 'scpexp' ) 
        x.expr  = strcat( 'max(', s1expr,  ',', s2expr,  ')' );
        x.vexpr = strcat( 'max(', s1vexpr, ',', s2vexpr, ')' );
    end
else
    if nargin == 3 && isa( varargin{1}, 'scpexp' ) ...
       && isempty( varargin(2) ) && isreal( varargin{3} )
        x = varargin{1};
        x.expr  = strcat( 'max(', x.expr,  ',[],', num2str(varargin{3}), ')' );
        x.vexpr = strcat( 'max(', x.vexpr, ',[],', num2str(varargin{3}), ')' );
    else
        isobject = false;
        tmp = varargin{1};
        if isa( tmp, 'scpexp' )
        ...
        if isobject
            x.expr  = strcat( 'max(', xexpr,  ')');
            x.vexpr = strcat( 'max(', xvexpr, ')');
        end
    end
end
