...
scp_begin quiet
    scp_variable x_nom(n)
    scp_minimize ( norm(A*x_nom - b) )
scp_end
%         (reduces to minimizing max{||(A-B)x - b||_2, ||(A+B)x - b||_2}
fprintf(1,'Done! \n');
...
P = (1/3)*B'*B;
scp_begin quiet
    scp_variable x_stoch(n)
    scp_minimize (  square_pos(norm(A*x_stoch - b)) + quad_form(x_stoch,P) )
scp_end
...
scp_begin quiet
    scp_variable x_wc(n)
    scp_minimize ( max( norm((A-B)*x_wc - b), norm((A+B)*x_wc - b) ) )
scp_end
