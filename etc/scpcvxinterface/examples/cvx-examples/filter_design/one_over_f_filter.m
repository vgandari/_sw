...
scp_begin
  scp_variable r(n,1)   % auto-correlation coefficients
  scp_variable R(m,1)   % power spectrum

  % log-chebychev minimax design
  scp_minimize( max( max( [R./(D.^2)  (D.^2).*inv_pos(R)]' ) ) )
  scp_subject to
...
disp(['Problem is ' scp_pinfo.status])
if ~strfind(scp_pinfo.status,'Solved')
  return
end
