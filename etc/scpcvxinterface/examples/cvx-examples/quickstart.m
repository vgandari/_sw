...
% Compare
disp( sprintf( '\nResults:\n--------\nnorm(A*x_ls-b): %6.4f\nnorm(A*x-b):    %6.4f\ncvx_optval:     %6.4f\nscp_pinfo.status:     %s\n', norm(A*x_ls-b), norm(A*x-b), scp_optval, scp_pinfo.status ) );
...
% Compare
if has_quadprog,
    disp( sprintf( '\nResults:\n--------\nnorm(A*x_qp-b): %6.4f\nnorm(A*x-b):    %6.4f\ncvx_optval:     %6.4f\nscp_pinfo.status:     %s\n', norm(A*x_qp-b), norm(A*x-b), scp_optval, scp_pinfo.status ) );
    ...
    disp( sprintf( '\nResults:\n--------\nnorm(A*x-b): %6.4f\ncvx_optval:  %6.4f\nscp_pinfo.status:  %s\n', norm(A*x-b), scp_optval, scp_pinfo.status ) );
...
% Compare
if has_linprog,
    disp( sprintf( '\nResults:\n--------\nnorm(A*x_lp-b,Inf): %6.4f\nnorm(A*x-b,Inf):    %6.4f\ncvx_optval:         %6.4f\nscp_pinfo.status:         %s\n', norm(A*x_lp-b,Inf), norm(A*x-b,Inf), scp_optval, scp_pinfo.status ) );
    disp( 'Verify that x_lp == x:' );
    disp( [ '   x_lp  = [ ', sprintf( '%7.4f ', x_lp ), ']' ] );
    disp( [ '   x     = [ ', sprintf( '%7.4f ', x ), ']' ] );
else,
    disp( sprintf( '\nResults:\n--------\nnorm(A*x-b,Inf): %6.4f\nscp_optval:      %6.4f\nscp_pinfo.status:      %s\n', norm(A*x-b,Inf), scp_optval, scp_pinfo.status ) );
    disp( 'Optimal vector:' );
    disp( [ '   x     = [ ', sprintf( '%7.4f ', x ), ']' ] );
end
disp( sprintf( 'Residual vector; verify that the peaks match the objective (%6.4f):', scp_optval ) );
disp( [ '   A*x-b = [ ', sprintf( '%7.4f ', A*x-b ), ']' ] );
disp( ' ' );
...
% Compare
if has_linprog,
    disp( sprintf( '\nResults:\n--------\nnorm(A*x_lp-b,1): %6.4f\nnorm(A*x-b,1): %6.4f\nscp_optval: %6.4f\nscp_pinfo.status: %s\n', norm(A*x_lp-b,1), norm(A*x-b,1), scp_optval, scp_pinfo.status ) );
    disp( 'Verify that x_lp == x:' );
    disp( [ '   x_lp  = [ ', sprintf( '%7.4f ', x_lp ), ']' ] );
    disp( [ '   x     = [ ', sprintf( '%7.4f ', x ), ']' ] );
else,
    disp( sprintf( '\nResults:\n--------\nnorm(A*x-b,1): %6.4f\nscp_optval: %6.4f\nscp_pinfo.status: %s\n', norm(A*x-b,1), scp_optval, scp_pinfo.status ) );
    disp( 'Optimal vector:' );
    disp( [ '   x     = [ ', sprintf( '%7.4f ', x ), ']' ] );
end
...
scp_begin
    scp_variable x(n);
    scp_minimize( norm_largest(A*x-b,k) );
scp_end
...
% Compare
temp = sort(abs(A*x-b));
disp( sprintf( '\nResults:\n--------\nnorm_largest(A*x-b,k): %6.4f\nscp_optval: %6.4f\nscp_pinfo.status: %s\n', norm_largest(A*x-b,k), scp_optval, scp_pinfo.status ) );
disp( 'Optimal vector:' );
disp( [ '   x     = [ ', sprintf( '%7.4f ', x ), ']' ] );
...
scp_begin quiet
    scp_variable x(n);
    scp_minimize( sum(huber(A*x-b)) );
scp_end

echo off

% Compare
disp( sprintf( '\nResults:\n--------\nsum(huber(A*x-b)): %6.4f\nscp_optval: %6.4f\nscp_pinfo.status: %s\n', sum(huber(A*x-b)), scp_optval, scp_pinfo.status ) );
disp( 'Optimal vector:' );
disp( [ '   x     = [ ', sprintf( '%7.4f ', x ), ']' ] );
disp( 'Residual vector:' );
disp( [ '   A*x-b = [ ', sprintf( '%7.4f ', A*x-b ), ']' ] );
...
for k = 1:length(gamma),
    ...
    scp_begin quiet
        scp_variable x(n);
        scp_minimize( norm(A*x-b)+gamma(k)*norm(x,1) );
    scp_end
    l1norm(k) = norm(x,1);
    l2norm(k) = norm(A*x-b);
    fprintf( 1, '   %8.4e   %8.4e\n', l1norm(k), l2norm(k) );
